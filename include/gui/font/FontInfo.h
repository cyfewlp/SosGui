...
struct FontInfo
{
    ...
    FontInfo(
        const std::string_view &filePath, const std::string_view &familyName, const uint32_t fontIndex, const bool bold = false,
    ...
    FontInfo(const FontInfo &other)
        : filePath(other.filePath), familyName(other.familyName), familyIndex(other.familyIndex), fontIndex(other.fontIndex),
          faceIndex(other.faceIndex), bold(other.bold), oblique(other.oblique)
    {
    }
    ...
    bool IsValid() const { return fontIndex >= 0 && filePath.size() > 0 && familyName.size() > 0; }
