...
{
    bool         editing_outfit_invalid = false;
    ...
    if (auto &runtime_data = inventory_manager->GetRuntimeData(); !runtime_data.loadedModels.empty())
    {
        ...
        if (const auto &loaded_sp_model = loaded_model.spModel; loaded_sp_model != nullptr)
        {
            ...
            if (ImGui::Begin(preview_window_name, nullptr, ImGuiEx::WindowFlags().NoResize().NoDecoration().NoBackground().NoFocusOnAppearing()))
            {
                ...
                {
                    if (!std::exchange(preview_armor_rotating_, true))
                    {
                        RE::Inventory3DManager::StartMouseRotation();
                    }
                }
                else if (std::exchange(preview_armor_rotating_, false))
                {
                    RE::Inventory3DManager::StopMouseRotation();
                }
...
void OutfitEditPanel::draw_armor_view_content(const EditingOutfit &editingOutfit, ArmorDrawContext &armor_draw_context)
{
    ...
    draw_preview_armor_window(armor_draw_context.to_preview_armor);

    draw_add_armors_popup(editingOutfit);
}

void OutfitEditPanel::draw_armor_view(const std::vector<ArmorEntry> &viewData, ArmorDrawContext &armor_draw_context)
{
        ...
        while (index != end && clipper.UserIndex < clipper.DisplayEnd)
        {
            ...
            if (armor_view_.filter(armor))
            {
                if (clipper.UserIndex >= clipper.DisplayStart)
                {
                    ...
                    draw_armor_row(index, armor, armor_draw_context);
                    ImGui::PopID();
                }
                clipper.UserIndex++;
            }
    ...
    if (armor_draw_context.want_add_armor)
    {
...
void OutfitEditPanel::draw_armor_row(const int row_index, const Armor *armor, ArmorDrawContext &armor_draw_context)
{
    ImGui::TableNextRow();
    ...
    {
        ImGui::BeginDisabled(armor_draw_context.editing_outfit_invalid || armor_view_.multi_selection_.Size <= 0);
